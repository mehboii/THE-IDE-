class AppController {
  ...
  openCustomModelModal(model = null) {
    ...
    ['name', 'host', 'port', 'type', 'apiKey'].forEach((key) => { if (model?.[key] != null) this.customModelForm.elements[key].value = model[key]; });
    if (model?.model) {
      const opt = document.createElement('option'); opt.value = model.model; opt.textContent = model.model; opt.selected = true;
  ...
  async fetchModelsForForm(options = {}) {
    ...
    try {
      ...
      if (result.success && result.models && result.models.length > 0) {
        ...
        if (currentValue) { const opt = document.createElement('option'); opt.value = currentValue; opt.textContent = `${currentValue} (unverified)`; opt.selected = true; modelSelect.appendChild(opt); }
        this.customModelTestResult.className = 'custom-model-test-result error';
